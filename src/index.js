...
class App {
  static resetButtonColors() {
    Array.from(document.getElementsByClassName("trip")).forEach(
      x => (x.style.backgroundColor = "#57BC90")
    );
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  tripList.addEventListener("click", function(event) {
    ...
    let eventId = parseInt(
      event.path.filter(x => x.className === "trip")[0].id
    );
  ...
  homeBtn.addEventListener("click", function(event) {
    ...
    document.getElementById("user-greeting").innerText = `Hello ${
      User.getById(user).name
    }!`;
    let userTotal = Trip.getByUser(user).length;
    document.getElementById(
      "total-trips"
    ).innerText = `You've been on ${userTotal} trips.`;
    document.getElementById("work-trips").innerText = Trip.getByWork(
      user
    ).length;
    document.getElementById("leisure-trips").innerText = Trip.getByLeisure(
      user
    ).length;
    document.getElementById("beach-trips").innerText = Trip.getByBeach(
      user
    ).length;
    document.getElementById("family-trips").innerText = Trip.getByFamily(
      user
    ).length;
    document.getElementById("friends-trips").innerText = Trip.getByFriends(
      user
    ).length;
  });
});
